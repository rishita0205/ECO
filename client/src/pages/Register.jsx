...
const Register = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const { data } = await axios.post('https://server-eco.onrender.com/user/register', { name, email, password });
      const userWithToken = {
        ...data.user,
        token: data.token,
      };
  ...
  return (
    <div className="flex items-center justify-center min-h-screen bg-gray-100">
      {showNotification && (
        <div className="fixed top-5 right-5 bg-red-500 text-white py-2 px-4 rounded-md shadow-lg z-50">
          {notificationMessage}
